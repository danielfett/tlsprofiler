...
class ProfileTests(unittest.TestCase):
    ...
    VALIDATION_ERRORS = ['validation not successful: unable to get local issuer certificate (trust store Android)',
    ...
    def test_modern_profile(self):
        profiler = TLSProfiler("modern.dev.intranet", PROFILE.MODERN, self.ECDSA_CA_FILE)
        result = profiler.run()
    ...
    def test_intermediate_profile(self):
        profiler = TLSProfiler("intermediate.dev.intranet", PROFILE.INTERMEDIATE, self.RSA_CA_FILE)
        result = profiler.run()
        self.assertEqual(result.validation_errors, self.VALIDATION_ERRORS)
        self.assertFalse(result.validated)
        self.assertEqual(result.profile_errors,
    ...
    def test_old_profile(self):
        ...
        self.assertEqual(result.profile_errors,
                         ['server has the wrong cipher suites order (Protocol TLSv1.3)',
                          'must support DES-CBC3-SHA', 'OCSP stapling must be supported'])
        self.assertFalse(result.profile_matched)
        self.assertEqual(result.vulnerability_errors, [])
        self.assertFalse(result.vulnerable)
        self.assertFalse(result.all_ok)
